            ...
            <v-slide-x-reverse-transition appear>
                <div class="d-flex">
                    <div v-if="pcOnly" class="d-flex align-center">
                        <v-btn :color="activeSection === link.name ? 'secondary' : undefined" :outlined="activeSection === link.name" v-for="link in links" :key="link.id" large tile :text="activeSection !== link.name" class="ms-2" @click="navigateToLink(link.ref)">{{ link.name }}</v-btn>
                    </div>
                </div>
